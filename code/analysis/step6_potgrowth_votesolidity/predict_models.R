...
for (i in parties){
  model <- readRDS(paste0("_SharedFolder_article_pot-growth/data/marts/step6_potgrowth_solidity/models/vote_solidity/model_", i, ".rds"))
...
for (i in parties){
  model <- readRDS(paste0("_SharedFolder_article_pot-growth/data/marts/step6_potgrowth_solidity/models/potgrowth/model_", i, ".rds"))
  Predsi <- marginaleffects::predictions(model, newdata = Strat,
                                         conf_level = 0.95) %>% 
    select(-starts_with("rci")) %>% 
    mutate(party = i,
...
model <- readRDS("_SharedFolder_article_pot-growth/data/marts/step6_potgrowth_solidity/models/model_voteint.rds")
...
saveRDS(PredsVoteInt, "_SharedFolder_article_pot-growth/data/marts/step6_potgrowth_solidity/model_predictions/voteint.rds")
...
saveRDS(Preds, "_SharedFolder_article_pot-growth/data/marts/step6_potgrowth_solidity/model_predictions/potgrowth_votesolidity.rds")
